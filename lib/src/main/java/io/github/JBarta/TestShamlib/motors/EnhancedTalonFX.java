...
public class EnhancedTalonFX implements Sendable {
    ...
    public EnhancedTalonFX(EnhancedTalonFXConfiguration config) {
        this.pro = config.type == EnhancedTalonFXConfiguration.TalonType.PRO;
        ...
        if(pro) {
            ...
            if(config.runMode == EnhancedTalonFXConfiguration.RunMode.MOTION_MAGIC) {
            ...
            } else if(config.runMode == EnhancedTalonFXConfiguration.RunMode.VELOCITY) {
                TalonFXConfiguration motorConfig = new TalonFXConfiguration();
    ...
    public void setTarget(double target) {
        ...
        if(pro) {
            if(runMode == EnhancedTalonFXConfiguration.RunMode.VELOCITY) {
                proMotorController.setControl(new VelocityVoltage(outputToDefaultUnits(target)).withSlot(0));
            } else if(runMode == EnhancedTalonFXConfiguration.RunMode.MOTION_MAGIC) {
    ...
    private void configurePro(EnhancedTalonFXConfiguration.NeutralBehavior nb, EnhancedTalonFXConfiguration.InvertedBehavior ib) {
        MotorOutputConfigs config = new MotorOutputConfigs();
        proMotorController.getConfigurator().refresh(config);
        config.NeutralMode = nb == EnhancedTalonFXConfiguration.NeutralBehavior.COAST ? NeutralModeValue.Coast : NeutralModeValue.Brake;
        config.Inverted = ib == EnhancedTalonFXConfiguration.InvertedBehavior.CCWP ? CounterClockwise_Positive : Clockwise_Positive;
        proMotorController.getConfigurator().apply(config);
    }

    private void configureV5(EnhancedTalonFXConfiguration.NeutralBehavior nb, EnhancedTalonFXConfiguration.InvertedBehavior ib) {
        v5MotorController.setNeutralMode(nb == EnhancedTalonFXConfiguration.NeutralBehavior.COAST ? NeutralMode.Coast : NeutralMode.Brake);
        v5MotorController.setInverted(ib == EnhancedTalonFXConfiguration.InvertedBehavior.CCWP ? TalonFXInvertType.CounterClockwise : TalonFXInvertType.Clockwise);
    }
