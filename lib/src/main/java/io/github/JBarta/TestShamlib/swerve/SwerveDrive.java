package io.github.JBarta.TestShamlib.swerve;
...
import io.github.JBarta.TestShamlib.PIDGains;
import io.github.JBarta.TestShamlib.swerve.module.SwerveModule;
import io.github.JBarta.TestShamlib.swerve.module.SwerveModuleConfiguration;
...
public class SwerveDrive {
     ...
     * @param swerveModuleInfos Array of module infos, one for each module
     */
    public SwerveDrive(int pigeon2ID,
                       ...
                       String gyroCanbus,
                       SwerveModuleConfiguration... swerveModuleInfos) {
        ...
        Translation2d[] offsets = new Translation2d[swerveModuleInfos.length];
        for(int i = 0; i< swerveModuleInfos.length; i++) {
            numModules++;
            SwerveModuleConfiguration m = swerveModuleInfos[i];

            offsets[i] = m.offset;
            modules.add(new SwerveModule(m));
        }
    ...
    public Command calculateTurnKV(Trigger increment, BooleanSupplier interrupt) {
        return modules.get(0).calculateTurnKV(increment, interrupt);
    }

    public Command calculateDriveKV(Trigger increment, Trigger invert, BooleanSupplier interrupt) {
        Command toRun = new InstantCommand();
        boolean first = true;
        for(SwerveModule module : modules) {
            toRun = toRun.alongWith(module.calculateDriveKV(increment, invert, interrupt, first));
